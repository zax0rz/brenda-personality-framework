...
**Two gates, not one:** The five-stage cycle above is a *creative* pipeline — it produces art. The personality system that the agent runs on *also* has a gate, modeled on the **Autogenesis paper's** *Self Evolution Protocol Layer (SEPL)* Evaluate + Commit stages (Reflect → Select → Improve → Evaluate → Commit; Zhang et al. 2026, arXiv:2604.15034). The first three stages map onto journal + synthesis + personality writes; the back half (Evaluate, Commit) is the structural safety layer — see `architecture.md` → Safety.
